...
请根据以下对话历史重构查询："""
    ...
    def _build_prompt(self, messages: List[Dict[str, str]], context: str) -> str:
        ...
        Returns:
            str: 完整的Prompt
        """
        # 将对话历史格式化为文本（排除最后一条用户消息，因为它会单独处理）
        ...
        prompt = f"""你是一个专业的新闻助手，请根据提供的新闻内容和对话历史回答用户的当前问题。
...
请按照以下要求回答：
1. 基于新闻内容和对话历史提供准确、客观的回答
2. 保持对话的连贯性，考虑对话历史中的上下文
3. 如果新闻内容中没有相关信息，请明确说明"根据提供的新闻内容，无法回答这个问题"
4. 回答要简洁明了，重点突出
5. 使用中文回答
6. 不要添加新闻内容中没有的信息
    ...
    def _call_llm_for_answer(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的新闻助手，根据新闻内容和对话历史回答问题。"},
        ...
        try:
            ...
            for article_id in relevant_article_ids:
                try:
                    ...
                    context_parts.append(f"【新闻ID: {article_id}】\n{content}\n")
                except ValueError:
